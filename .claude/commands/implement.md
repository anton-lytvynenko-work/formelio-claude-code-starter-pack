...
## Greenfield Pre-Task: Scaffold Stack

**Before any feature work**, check whether the plan's first task is `Task 1: Scaffold stack`. If so:

1. Confirm no stack file exists: `ls package.json pyproject.toml Cargo.toml go.mod 2>/dev/null` → expect empty
2. Read the stack chosen in the plan's "Tech Stack" / Summary section
3. **Invoke the `project-init` skill** with that stack — it runs Phases 3–8 of its workflow:
   - Phase 3: copy stack-specific templates from `.claude/skills/project-init/templates/<stack>/`
   - Phase 4: append stack entries to `.gitignore`
   - Phase 5: prune hooks irrelevant to the stack (e.g. drop TypeScript-only hooks for Python)
   - Phase 6: fill remaining `{{STACK}}`, `{{INSTALL_COMMAND}}`, etc. placeholders in `CLAUDE.md`, `AGENTS.md`, `README.md`, `docs/setup.md`, `docs/architecture.md`
   - Phase 7.5: write `.claude/rules/custom/project.md` with real stack facts and directory tree
   - Phase 7.6: if `.mcp.json` exists, write `.claude/rules/custom/mcp-tools.md`
   - Phase 8: validate (no unreplaced placeholders, config files parse)
4. Optionally run the install command (ask the user via `AskUserQuestion`)
5. **Mark Task 1 complete** in the plan (`[ ]` → `[x]`) and update progress counts

Only after stack scaffold is done, proceed to feature tasks (Task 2 onward) using the normal flow below.

If the plan's Task 1 is **not** "Scaffold stack" but no stack file exists, **STOP** and inform the user — the plan is incomplete (Phase 3 of `/plan` mandates a scaffold task for greenfield projects).

If a stack file already exists and Task 1 is "Scaffold stack", **STOP** and inform the user — the plan is stale (someone scaffolded outside this workflow). Ask whether to skip the task or regenerate.
...
### STEP 11b: ARCHITECTURE CHECK (STRUCTURAL plans only)

11b. **If the plan's Architecture Impact is STRUCTURAL AND this task created new files:**
    - Update `docs/architecture.md` Directory Map now (add the new file)
    - This is incremental — the final Architecture task handles diagrams and flows
    
    **Skip for NON-STRUCTURAL plans.**
...
1b. **STRUCTURAL plans only — Architecture verification:**
   - Read `docs/architecture.md` — does Directory Map include all new files from this plan?
   - Read each diagram listed in Architecture Impact — does it match implemented code?
   - If anything missing: fix NOW before marking COMPLETE
